...
namespace Extend\WarrantyGraphQl\Model\Resolver\Product;
...
class PriceRange implements ResolverInterface
{
    ...
    )
    {
        ...
        if (isset($requestedFields['minimum_price'])) {
            $returnArray['minimum_price'] = $this->getMinimumProductPrice($value, $product, $store);
        }
        if (isset($requestedFields['maximum_price'])) {
            $returnArray['maximum_price'] = $this->getMaximumProductPrice($value, $product, $store);
        }
        return $returnArray;
    }
     ...
     * @return array
     */
    private function getMinimumProductPrice($value, SaleableInterface $product, StoreInterface $store): array
    {
        $regularPrice = $value['price'] ?? $product->getPrice();
        $finalPrice = $value['price'] ?? $product->getPrice();
    ...
    private function getMaximumProductPrice($value, SaleableInterface $product, StoreInterface $store): array
    {
        $regularPrice = $value['price'] ?? $product->getPrice();
        $finalPrice = $value['price'] ?? $product->getPrice();
